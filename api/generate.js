...
.product-card{background:#fff;border:1px solid var(--g200);border-radius:20px;overflow:hidden;transition:all .18s;cursor:pointer}
.product-card:hover{transform:translateY(-3px);box-shadow:0 12px 32px rgba(30,58,95,.1);border-color:var(--g300)}
.product-card-img{height:160px;background:var(--g100);position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center;font-size:52px}
...
.product-card-why{font-size:12px;color:var(--g600);line-height:1.55;margin-bottom:12px;padding:10px;background:var(--g50);border-radius:10px;border-left:3px solid var(--gold)}
...
<!-- ═══════════════════════════════════════════════════════ -->
<!-- VIEW 2: FULL PRODUCT ANALYSIS (shown when Analyse clicked) -->
<!-- ═══════════════════════════════════════════════════════ -->
    ...
    <div class="admin-panel" id="admin-panel">
      <div class="admin-card">
        ...
        <div class="status-messages" id="status-messages">
          ...
          <div class="status-line" id="sl-6">Running second product batch...</div>
          <div class="status-line" id="sl-7">Combining and deduplicating results...</div>
...
function renderProductsGrid() {
  ...
  grid.innerHTML = list.map((p, i) => `
    <div class="product-card" onclick="openAnalysis(${i})">
      <div class="product-card-img" style="background:${p.bgColor||'#F3F4F6'}">
        <span>${p.emoji || '📦'}</span>
  ...
  body.innerHTML = `
    ...
    <div class="analysis-panel">
      <div class="analysis-panel-head">
        ...
        <h2>Growth signal — last 6 weeks</h2>
      </div>
    ...
    <div class="analysis-panel">
      ...
      <div class="analysis-panel-body">
        <p style="font-size:13px;color:var(--g500);margin-bottom:14px">Check who's already selling this, what ads are running and how search is trending. Use multiple searches to get the full picture.</p>
    ...
    <div class="analysis-panel">
      <div class="analysis-panel-head">
        <span style="font-size:20px">🎬</span>
        <h2>Ad creative generator — 3 variations</h2>
      </div>
      <div class="analysis-panel-body">
...
async function runValidator() {
  ...
  try {
    ...
    // Build a fake product object and show full analysis
    const p = {
    ...
    // Inject full analysis into validator result area
    ...
    // Reuse buildAnalysisBody
    const savedBody = document.getElementById('analysis-body');
    buildAnalysisBody(p);
...
function extractProductName(url) {
  try {
    // Try to get meaningful words from the URL path
...
async function adminGenerate() {
  ...
  try {
    // Call API twice with 10 products each to stay within timeout
    status.textContent = 'Fetching batch 1 of 2...';
    ...
    async function fetchBatch(batchNum) {
      ...
      if (s === -1 || e === -1) throw new Error('No array in batch ' + batchNum);
      return JSON.parse(c.slice(s, e + 1));
    }

    const batch1 = await fetchBatch(1);
    status.textContent = `Batch 1 done (${batch1.length} products). Fetching batch 2...`;
    const batch2 = await fetchBatch(2);
    status.textContent = `Batch 2 done. Processing...`;
    
    // Combine and deduplicate by name
    const seen = new Set();
    const products = [...batch1, ...batch2].filter(p => {
    ...
    // Mark remaining lines as done
    lines.forEach(id => {
      const el = document.getElementById(id);
      if(el) el.className = 'status-line done show';
    });
...
function publishProducts() {
  ...
  // Only include toggled-on products
