  ...
  grid.innerHTML = list.map((p, i) => `
    <div class="product-card" onclick="openAnalysis(${i})">
      <div class="product-card-img" style="background:${p.bgColor||'#F3F4F6'}">
        ${p.img ? `<img src="${p.img}" alt="${p.name}" style="width:100%;height:100%;object-fit:cover;display:block" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span style="display:none">${p.emoji||'📦'}</span>` : `<span>${p.emoji||'📦'}</span>`}
...
async function adminGenerate() {
  ...
  try {
    ...
    const products = JSON.parse(c.slice(s, e + 1));
